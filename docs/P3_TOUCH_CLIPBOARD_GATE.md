...
移动端终端交互核心闭环已在 Android 15/API 35 ARM64 真机通过：单指滑动向远端
WezTerm pane 发送真实鼠标滚轮事件，双指滑动浏览本地 scrollback；长按字符网格进入
选择模式并拖动扩展，复制到 Android 系统剪贴板，再把剪贴板内容粘贴回当前 SSHMUX
pane。功能直接作用于 WezTerm cell/稳定历史行，不是对截图或 Android TextView 做选择。

| 检查项 | 状态 | 实机结果 |
|---|---|---|
| 单指远端滚轮 | PASS | 单指上滑产生 9 次 SSHMUX mouse-wheel dispatch，本地历史按钮未出现 |
| 双指本地回滚 | PASS | 两指轨迹回滚到实时底部上方 3 行，远端 mouse-wheel dispatch 为 0 |
...
TerminalSurfaceView GestureDetector
  ├─ one-finger drag / fling -> Pane::mouse_event -> remote TUI
  ├─ two-finger drag / fling -> nativeScrollByRows
...
2. 双指向下拖动终端，状态变为距实时底部 12 行，画面显示 `74–90`；
...
- `wezterm-android-core` 12 个测试通过，包括 scrollback 边界、宽字符和跨行文本抽取；
- `wezterm-android-mux` 6 个测试通过，包括 Android 底部对齐视口、滚轮方向和断线事件；
- font/ssh 合计 11 个测试通过；四个交付 crate 共 29 个测试通过；
- Kotlin/JVM 11 个测试通过，包括 3 个单双指滚动方向与余量隔离测试；
...
- SSHMUX 鼠标报告与本地历史的单双指仲裁已完成；普通 SSH 当前以方向键作为
  单指远端滚动的兼容回退；

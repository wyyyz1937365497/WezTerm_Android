...
impl SessionInner {
    ...
    fn run_impl_libssh(&mut self) -> anyhow::Result<()> {
        ...
        if let Some(ssh_dir) = self.config.get("wezterm_ssh_dir") {
            sess.set_option(libssh_rs::SshOption::SshDir(Some(ssh_dir.clone())))?;
        }
        let process_default_config = self
            .config
            .get("wezterm_ssh_process_config")
            .map(|value| value.parse::<bool>())
            .transpose()
            .context("wezterm_ssh_process_config must be true or false")?
            .unwrap_or(true);
        if process_default_config {
            sess.options_parse_config(None)?;
        } else {
            // Prevent both this setup path and libssh's later ssh_connect()
            // call from consulting ~/.ssh/config or /etc/ssh/ssh_config.

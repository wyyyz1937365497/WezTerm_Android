...
#[cfg(target_os = "android")]
static ANDROID_HOME_DIR: OnceLock<PathBuf> = OnceLock::new();

/// Supplies the app-private home directory on Android, where there is no
/// passwd database entry for an application UID.
///
/// Repeating the same value is accepted; changing it after initialization is
/// rejected so that upstream lazy configuration paths remain consistent.
#[cfg(target_os = "android")]
pub fn set_android_home_dir(path: PathBuf) -> bool {
    match ANDROID_HOME_DIR.get() {
        Some(existing) => existing == &path,
        None => ANDROID_HOME_DIR.set(path).is_ok(),
    }
}

#[cfg(target_os = "android")]
fn android_home_dir() -> Option<PathBuf> {
    ANDROID_HOME_DIR.get().cloned()
}
